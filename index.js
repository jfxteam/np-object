  ...
  static deepAssign(...args){
    return args.filter(v => v).reduce((assigned, obj, index, array) => {
      ...
      this.keys(obj).map(key => {
        ...
        try {
          valueInstanceOfNode = (value instanceof Node);
        } catch(e){
          valueInstanceOfNode = false;
        }
        
        if(typeof(value) === 'object' && value !== null && !valueInstanceOfNode && this.keys(value).length && !Array.isArray(value)){
